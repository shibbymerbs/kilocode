...
export function buildWebviewHtml(
  ...
  opts: {
    ...
    port?: number
    extraStyles?: string
    /** Sidebar top bar visibility for the shared webview bundle (App.tsx). Unused by the Agent Manager bundle. */
    topBar?: boolean
...
<body>
  <div id="root"></div>
  <script nonce="${nonce}">window.ICONS_BASE_URI = "${opts.iconsBaseUri}"; window.KILO_SHIKI_WORKER_URI = "${opts.workerUri}"; window.KILO_MARKDOWN_SHIKI_WORKER_URI = "${markdownWorkerUri}"; window.KILO_TOP_BAR = ${opts.topBar !== false}; window.KILO_AGENT_MANAGER_SETTINGS = ${opts.agentManagerSettings === true}; window.KILO_BROWSER_AUTOMATION = ${opts.browserAutomation === true}; window.KILO_AGENT_MANAGER_INTRO_DISMISSED = ${opts.introDismissed === true};</script>
